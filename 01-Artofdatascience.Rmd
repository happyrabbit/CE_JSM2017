...
How does DSI define Data science? The web site for DSI gives us an idea:
...
- Question 2:  Is the new promotional tool introduced at the beginning of this year boosted the annual sales of P1197 in Iowa and Wisconsin? (P1197 is an impressive corn seed product from DuPont Pioneer)
...
-  Are the purchasing amount different between consumers received coupons and those didn't have coupons?
- Is male more inclined to buy our products than female?
...
- Which printer performs similar pattern to the broken ones? (QC)
-  How many different kinds of employees are there in the company? (Human Resources)
- How many different themes are there in the corpus? (Natural semantic processing)

Note that clustering is unsupervised learning. The most common algorithms are K-Means and Hierachical Clustering. 

4. Classification

Usually, a labeled sample set is used as a training set to train the classifier. Then use the classifier to predict the category of a future sample.  Here are some example questions:
...
Most of the data scientists today have undergraduate or higher degree from one of the following areas: computer science, electronic engineering, mathematics or statistics. According to a 2015 survey, 48% of US data scientists have a doctoral degree, 44% have a master degree, and only 8% are undergraduate. 
...
The least squares regression and logistic regression are traditional statistical models. Both of them are highly interpretable. MARS is similar to neural networks and partial least squares in the respect that they all use surrogate features instead of original predictors.  
They differ in how to create the surrogate features. PLS and neural networks used linear combinations of the original predictors as surrogate features. MARS creates two contrasted versions of a predictor by a truncation point. And LOESS is a non-parametric model, usually only used in visualization.
...
The biggest advantage of this type of model is that they are intuitive.  The key is to find the appropriate distance metric.  K-Nearest Neighbour is the most popular one. The other two are not heard often in the application. 
