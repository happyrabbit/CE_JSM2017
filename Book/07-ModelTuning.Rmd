...
Instead of evaluating model on a single test set, more often, people will resample the training set. _Data resampling_ operates similarly: use a subset of samples to fit a model and use the rest to evaluate model performance. This process will repeat multiple times to get a performance profile. In that sense, resampling is based on splitting. The general steps are:
...
![](http://scientistcafe.com/book/Figure/ParameterTuningProcess.png){width=100%}